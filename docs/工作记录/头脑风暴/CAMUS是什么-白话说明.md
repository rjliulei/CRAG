...
> Related Work 差界白话：[差界三轴](#差界三轴写-related-work用) · 精读稿：[12-CAMUS精读论文解析](../../论文/12-CAMUS精读论文解析.md)
...
## 差界三轴（写 Related Work 用）

审稿人问「这不就是××吗」时，钉死三句：**我们选证据的标准，跟这三类都不一样。**

同一道题想：*「2017 年之后的皮克斯电影有哪些？」* 检索出 20 句，混着相关但害人的（如把 Toy Story 4 塞进来）。

### 轴 1 · ≠ 固定 top-k / cosine / MMR

- **别人：** 谁跟问题「长得像」就留谁，前 10/20 全塞进模型。  
- **问题：** 像 ≠ 有用；噪声句也可以很像，照样把本会答对的题带偏。  
- **CAMUS：** 不按「像不像」堆句，看这句话有没有帮你**盖住问题硬约束**（年份、名单边界等）。  
- **一句话：** 相关分排序 ≠ 答题有没有用。

### 轴 2 · ≠ 熵减 / Information Gain（IGP 等）

- **别人：** 加这句话后模型「更笃定」（熵↓ / IG↑）→ 当好证据。  
- **问题：** 误导句也能让模型**错得更自信**（名单越编越长，熵照样降）。  
- **CAMUS：** 用约束覆盖 ΔU，不是「慌不慌」；用 **SAFE-U** 同池对比 IG 选证 vs 约束 ΔU。  
- **一句话：** 模型更自信 ≠ 答案更对。

### 轴 3 · ≠ RL / 重训选择器（Context-Picker 等）

- **别人：** 也要「少而够」的证据子集，但用强化学习 / LOO **训一个选证模型**。  
- **问题：** 要训练、多前向；你们 8B 无训、要周内可跑。  
- **CAMUS：** 同样反对固定 Top-K，落地是**规则贪婪 + 早停**，不训 picker。  
- **一句话：** 最小充分子集的想法可以同，落地是规则不是再训选择器。

### 合表

| 别人 | 他们选证看什么 | CAMUS 差在哪 |
|------|----------------|--------------|
| top-k / cosine / MMR | 像不像 | 看约束覆盖效用 |
| IGP / InfoGain | 模型更笃定吗 | 看答对向的 ΔU；对照证明 ≠ 熵减 |
| Context-Picker | 学一个选子集策略 | 无训练贪婪 + 早停 |

**记忆锚：** 轴1 → 朴素 RAG/重排；轴2 → **IGP**；轴3 → **Context-Picker**。  
Astute 偏「答时如何融合内外知识」，不是这三轴里的「选句标准」——另写干预点不同即可。

精读侧（含可贴 Related Work 的差界句）：[12-CAMUS精读论文解析.md](../../论文/12-CAMUS精读论文解析.md)。
